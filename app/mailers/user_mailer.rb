class UserMailer < PostageApp::Mailer
  ...
  def done_reminder_for(user, use)
    ...
    mail :to => { user.email => user.email_variables.merge(use.email_variables) }, :subject => ''
  end

  def available_reminder_for(user, resource)
    postageapp_template 'available'
    mail :to => { user.email => user.email_variables.merge(resource.email_variables) }, :subject => ''
  end
